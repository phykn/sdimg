...
class UniversalStripeRemover:
    ...
    def process_tiled(
        ...
        if n <= 1:
            return self.process(
                image=data, iterations=iterations, tol=tol, proj=proj, verbose=verbose
            )

        h, w = data.shape
        data = self._reflect_pad(
            t=data, pad_bottom=(n - h % n) % n, pad_right=(n - w % n) % n
        )
    ...
    def _fwd(
        ...
        return x.diff(
            dim=dim, append=x.narrow(dim=dim, start=x.size(dim) - 1, length=1)
        )
    ...
    def _cosine_window(
        ...
        if ov > 0:
            ramp = 0.5 * (
                1.0 - torch.cos(input=torch.linspace(start=0, end=math.pi, steps=ov))
            )
            win[:ov, :] *= ramp[:, None]
